    ...
    selectByEmbedId: function( id, node_id, name, useNode )
    {
        ...
        if ( id !== undefined )
        {
            var s = tinyMCEPopup.editor.settings, 
                    type = useNode === true && node_id > 0 ? 'eZNode_' + node_id : 'eZObject_' + id;
            var topDocument = top.document;
            var id = jQuery("iframe.uploadFrame_xrowGIS", topDocument).attr('id').substring( 12 )
                idArray = id.split('_');
            
            jQuery('#mceModalBlocker', topDocument).remove();
            window.parent.jQuery().servemap('addRelation', {'node_id':node_id, 'attributeID':idArray[4], 'version':idArray[2]});
            jQuery('#mce_' + idArray[3], topDocument).remove();
        }
    },
